...
use ssz_types::{
    ...
    typenum::{U4, U64, U2048, U8192, U65536, U262144, U16777216, U134217728},
};
...
impl BeaconState {
    ...
    pub fn get_beacon_proposer_index(&self, slot: Option<u64>) -> anyhow::Result<u64> {
        // Fulu uses pre-computed proposer lookahead
        let slot = slot.unwrap_or(self.slot);
        let index = (slot % SLOTS_PER_EPOCH) as usize;
        self.proposer_lookahead
            .get(index)
            .copied()
            .ok_or_else(|| anyhow!("Proposer lookahead index out of bounds: {index}"))
    }
    ...
    /// Convert Fulu BeaconState to Electra BeaconState.
    /// This drops the proposer_lookahead field as Electra computes proposers dynamically.
    pub fn to_electra(&self) -> crate::electra::beacon_state::BeaconState {
        crate::electra::beacon_state::BeaconState {
            ...
            previous_epoch_participation: self.previous_epoch_participation.clone(),
            current_epoch_participation: self.current_epoch_participation.clone(),
            ...
            current_sync_committee: self.current_sync_committee.clone(),
            ...
            deposit_requests_start_index: self.deposit_requests_start_index,
            deposit_balance_to_consume: self.deposit_balance_to_consume,
            exit_balance_to_consume: self.exit_balance_to_consume,
            earliest_exit_epoch: self.earliest_exit_epoch,
            consolidation_balance_to_consume: self.consolidation_balance_to_consume,
            earliest_consolidation_epoch: self.earliest_consolidation_epoch,
