...
impl BeaconApiClient {
    ...
    {
        Self::get_header_str(headers, key)?
            .parse::<T>()
    ...
    pub async fn produce_block(
        ...
        let content_type = Self::get_header_str(headers, "content-type")?;

        let version = Self::get_header_str(headers, "Eth-Consensus-Version")?.to_string();
        let execution_payload_blinded =
            Self::parse_header::<bool>(headers, "Eth-Execution-Payload-Blinded")?;
        let execution_payload_value =
            Self::parse_header::<u64>(headers, "Eth-Execution-Payload-Value")?;
        let consensus_block_value =
            Self::parse_header::<u64>(headers, "Eth-Consensus-Block-Value")?;
